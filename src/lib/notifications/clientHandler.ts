...
export async function applyNotificationAction(payload: NotificationActionPayload): Promise<void> {
...
async function markCompleted(reminder: Reminder, scheduledFor: number): Promise<void> {
  const now = Date.now();
  await addEvent({
...
async function markProgressPlusOne(reminder: Reminder, scheduledFor: number): Promise<void> {
  const unit =
...
async function markSkipped(reminder: Reminder, scheduledFor: number): Promise<void> {
  await addEvent({
    reminderId: reminder.id,
    action: "skipped",
    triggeredAt: Date.now(),
 ...
 */
export function consumeUrlNotifAction(href?: string): NotificationActionPayload | null {
