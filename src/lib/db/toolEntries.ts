...
export async function listToolEntries(options?: {
  ...
  if (options?.toolKey) {
    stored = await db.getAllFromIndex("tool_entries", "byToolKey", options.toolKey);
...
export async function listToolEntriesForDay(toolKey: ToolKey, day: string): Promise<ToolEntry[]> {
  const db = await getDB();
  const stored = await db.getAllFromIndex("tool_entries", "byLoggedAtDay", day);
  return stored
...
export async function sweepExpiredToolEntries(now: number = Date.now()): Promise<number> {
  try {
    ...
    const expired = all.filter((e) => e.expiresAt !== undefined && e.expiresAt <= now);
