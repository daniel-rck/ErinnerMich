...
export async function listMoodEntriesInRange(fromMs: number, toMs: number): Promise<MoodEntry[]> {
  const db = await getDB();
...
export async function listMoodEntriesForDay(day: string): Promise<MoodEntry[]> {
  const db = await getDB();
  const stored = await db.getAllFromIndex("mood_entries", "byLoggedAtDay", day);
  return stored.map(stripStored).sort((a, b) => b.loggedAt - a.loggedAt);
}
