...
export async function addEvent(input: NewReminderEvent): Promise<ReminderEvent> {
...
export async function listEventsForReminder(reminderId: string): Promise<ReminderEvent[]> {
  const db = await getDB();
...
export async function listEventsForDay(day: string): Promise<ReminderEvent[]> {
  const db = await getDB();
...
export async function dailyProgress(
  ...
  const db = await getDB();
  const events = await db.getAllFromIndex("events", "byTriggeredAtDay", day);
  const filtered = events.filter((e) => e.reminderId === reminderId).map(stripStored);
