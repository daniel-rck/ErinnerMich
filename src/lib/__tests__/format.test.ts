...
describe('formatSchedule', () => {
  ...
  it('beschreibt daily', () => {
    expect(
      formatSchedule({ type: 'daily', times: ['20:00', '08:00'] }),
    ).toContain('08:00')
  })

  it('beschreibt weekly', () => {
    expect(
      formatSchedule({ type: 'weekly', days: ['MON', 'FRI'], time: '19:00' }),
    ).toBe('Mo / Fr um 19:00')
  })
  ...
  it('beschreibt inventory_based', () => {
    expect(formatSchedule({ type: 'inventory_based' })).toBe(
      'Wenn Bestand niedrig',
    )
  })
})
...
describe('formatRelativeDate', () => {
  ...
  it('heute', () => {
    expect(
      formatRelativeDate(new Date('2026-05-04T08:00:00'), now),
    ).toBe('heute')
  })
  it('morgen', () => {
    expect(
      formatRelativeDate(new Date('2026-05-05T08:00:00'), now),
    ).toBe('morgen')
  })
  it('in 5 Tagen', () => {
    expect(
      formatRelativeDate(new Date('2026-05-09T12:00:00'), now),
    ).toBe('in 5 Tagen')
  })
})
