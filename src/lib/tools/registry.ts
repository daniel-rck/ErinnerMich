...
export const TOOL_BY_KEY = Object.fromEntries(TOOLS.map((t) => [t.key, t] as const)) as Record<
  ToolKey,
  ToolDef
>;

export const ACUTE_TOOL_KEYS: ToolKey[] = TOOLS.filter((t) => t.category === "acute").map(
  (t) => t.key,
);
