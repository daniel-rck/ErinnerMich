...
export function moodOverview(
  ...
  const avgEnergy =
    energy.length > 0 ? energy.reduce((a, b) => a + b, 0) / energy.length : null

  const series = dailyMoodSeries(entries, windowDays, today).filter(
    (p) => p.avgMood !== null,
  )
  ...
  return {
    ...
    avgEnergy,
    bestDay:
      best && best.avgMood !== null ? { day: best.day, avg: best.avgMood } : null,
    worstDay:
      worst && worst.avgMood !== null
        ? { day: worst.day, avg: worst.avgMood }
        : null,
  }
}
