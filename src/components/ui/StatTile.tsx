...
export const StatTile = forwardRef<HTMLDivElement, StatTileProps>(function StatTile(
  { label, value, trend, icon: Icon, accent = 'brand', size = 'md', spark, ariaLabel, onClick, className = '' },
  ref,
) {
  const TrendIcon =
    ...
    <>
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 text-[length:var(--text-micro)] tracking-[var(--tracking-caps)] uppercase font-medium text-[color:var(--color-text-tertiary)]">
          <span className={['inline-block h-1.5 w-1.5 rounded-full', ACCENT_DOT[accent]].join(' ')} />
          {label}
        </div>
        {Icon && <Icon size={16} aria-hidden className="text-[color:var(--color-text-tertiary)]" />}
      </div>
      <div className={[VALUE_SIZE[size], 'mt-1 tabular-nums text-[color:var(--color-text-primary)]'].join(' ')}>
        {value}
      </div>
      {(trend || spark) && (
        <div className="mt-1.5 flex items-end justify-between gap-2">
          {trend ? (
            <div className={['inline-flex items-center gap-0.5 text-[length:var(--text-caption)] font-medium', trendColor].join(' ')}>
